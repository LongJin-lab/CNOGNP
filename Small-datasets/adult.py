...
# URL of the training data (or path to your local file)
data_url = '/home/ps/media2/qingchuan/CNOHP/data/adult.data'

print(f"Loading data from {data_url}")
# Load the dataset, handling missing values ('?')
...
y = df['income'].str.strip() # Remove potential whitespace
...
# We use handle_unknown='ignore' to handle unseen categories during testing
...
print("Preprocessing finished. Processed data shape:", X_processed.shape)


# Encode the target variable ('<=50K'/' >50K')
# Note: The dataset uses '>50K' and '<=50K'. Strip whitespace.
label_encoder = LabelEncoder()
y_encoded = label_encoder.fit_transform(y) # ' <=50K' -> 0, ' >50K' -> 1

# Get the number of classes (should be 2)
num_classes = len(label_encoder.classes_) # This will be 2
...
X_train, X_test, y_train, y_test = train_test_split(X_processed, y_encoded, test_size=0.2, stratify=y_encoded) # Added stratify


# Convert to PyTorch tensors
# X_train and X_test from ColumnTransformer are typically numpy arrays (often sparse for one-hot encoding, but default is dense)
# Ensure they are dense numpy arrays if sparse, then convert
if isinstance(X_train, np.ndarray):
    X_train_tensor = torch.tensor(X_train, dtype=torch.float32).to(device)
else: # Handle potential sparse output (though default is dense)
...
# Target tensors should be Long for CrossEntropyLoss
...
if __name__ == "__main__":
    ...
    while True: # <-- Added: Start retry loop
        try:
            excel_dir='log/adult/{}/{}.xlsx'.format(args.loc,args.loc) #正常对比实验
