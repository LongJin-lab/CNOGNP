...
import time
import os 
from torch.autograd.functional import hessian # Add this import
# --- Argument Parsing ---
parser = argparse.ArgumentParser(description='Run CNO or CNOGNP optimization on Breast Cancer dataset.')

parser.add_argument('--algorithm', type=str, default='CNO', choices=['CNO', 'CNOGNP', 'PSO', 'CNOHEP', 'PSO-BFGS'],
...
parser.add_argument('--mu_hep', type=float, default=0.001, # Choose a reasonable default
                    help='Weight parameter for the maximum Hessian eigenvalue in CNOHEP (default: 0.001). Only used if --algorithm is CNOHEP.')
...
y_train_tensor = torch.tensor(y_train, dtype=torch.float32).unsqueeze(1).to(device) # Add .to(device)
X_test_tensor = torch.tensor(X_test_scaled, dtype=torch.float32).to(device) # Add .to(device)
y_test_tensor = torch.tensor(y_test, dtype=torch.float32).unsqueeze(1).to(device) # Add .to(device)
...
class BaseParticle:
    def __init__(self, model_template, device):
        ...
        self.model = model_template(input_dim=input_dim, hidden_dim=args.hidden_dim).to(self.device) # Add .to(self.device)
...
class ParticleCNOHEP(BaseParticle):
    def __init__(self, model_template, mu_hep, device):
        """
        Initializes a ParticleCNOHEP.

        Args:
            model_template: A function that returns a new instance of the MLP model.
            mu_hep: Regularization parameter for the Hessian eigenvalue term.
        """
        super().__init__(model_template, device)
        self.mu_hep = mu_hep # Regularization parameter for Hessian max eigenvalue
        # Store the max eigenvalue calculated in the last step (optional but can be useful)
        self.hessian_max_eigval = 0.0

    def _calculate_hessian_max_eigval(self, X, y):
        """
        Calculates the maximum eigenvalue of the Hessian of the loss with respect to the parameters.
        Assumes self.model parameters are already set to self.position.
        """
        # Define a function that takes the parameter vector and returns the loss
        # This is required by torch.autograd.functional.hessian
        def loss_fn_for_hessian(param_vector):
            # Temporarily set model parameters from the vector
            self.model._set_parameters_from_vector(param_vector)
            # Calculate loss
            self.model.train() # Ensure train mode for consistent behavior with loss calculation
            outputs = self.model(X)
        ...
        try:
            ...
            # inputs=self.position is the point at which to calculate the Hessian
            hessian_matrix = hessian(loss_fn_for_hessian, self.position, create_graph=False)

            # Ensure the Hessian matrix is a single tensor (it should be for R^n -> R)
            # If it's a tuple of tensors, this might need adjustment depending on pytorch version/usage
            # But for a flattened parameter vector to a scalar loss, it should be one tensor.
            if not isinstance(hessian_matrix, torch.Tensor):
                 # This might happen if the function maps to multiple outputs, or inputs are structured
                 # For our case (flattened vector -> scalar loss), it should be a single tensor
                 print("Warning: Hessian calculation did not return a single tensor. Check input/output shape.")
                 # Attempt to handle, e.g., if it's a tuple of one tensor
                 if isinstance(hessian_matrix, (list, tuple)) and len(hessian_matrix) == 1:
                     hessian_matrix = hessian_matrix[0]
                 else:
                     # Fallback or error handling if unexpected format
                     print(f"Error: Unexpected Hessian format: {type(hessian_matrix)}")
                     return float('nan') # Indicate failure


            # The Hessian of a scalar loss with respect to parameters is symmetric
            # Use eigvalsh for symmetric matrices for better stability and performance
            # Move to CPU if GPU memory is a concern for large Hessians, but keep on device if possible
            # Ensure the matrix is on the correct device
            hessian_matrix = hessian_matrix.to(self.device)
            eigenvalues = torch.linalg.eigvalsh(hessian_matrix)

            # Find the maximum eigenvalue
            max_eigval = torch.max(eigenvalues)

            return max_eigval.item() # Return as a scalar Python number

        except Exception as e:
             # Catch potential errors during Hessian calculation (e.g., memory issues, numerical errors)
             print(f"Error during Hessian calculation: {e}")
             # Return a large value or NaN to penalize the particle
             return float('nan') # Or float('inf') depending on desired behavior on failure


    def calculate_fitness(self, X, y): # Now takes data X, y
        """
        Fitness for CNOHEP is training loss f(z^i) + mu * lambda_max(H(f(z^i))).
        This method is called AFTER the position update in the step method.
        """
        # Ensure model parameters are set to the current position
        self.model._set_parameters_from_vector(self.position)
        self.model.train() # Set model to training mode

        # 2. Calculate the maximum Hessian eigenvalue at the current position
        # The helper method will set parameters internally, but let's be explicit just in case.
        # Ensure the model is on the correct device before setting params
        self.model.to(self.device)
        max_eigval = self._calculate_hessian_max_eigval(X, y)
        self.hessian_max_eigval = max_eigval # Store it

        # Handle potential calculation failures (e.g., NaN from Hessian)
        if np.isnan(self.hessian_max_eigval):
             # Decide how to handle failure - e.g., assign a very high fitness to penalize
             self.fitness_value = float('inf')
             print(f"Warning: Hessian max eigenvalue calculation failed for particle. Fitness set to inf.")
        else:
             # 3. Calculate fitness
             self.fitness_value = self.training_loss + self.mu_hep * self.hessian_max_eigval

        return self.fitness_value
        
    # Override the step method because the fitness calculation needs data (X, y)
    # and involves different components (loss + Hessian eigval) calculated after pos update.
    def step(self, global_best_position, omega, eta, c1, c2, X_train, y_train):
        """Perform one optimization step (lines 7-16 from the algorithm)."""
        # Line 7: Gradient step - calculate gradient at the *current* position (before velocity/position update)
        nabla_f = self.particle_forward_backward(X_train, y_train)
        ...
        # This call now uses the self.training_loss and self.model.parameters().grad
        # that were just populated by calculate_loss_and_populate_gradients.
        self.calculate_fitness(X_train, y_train)
...
def run_optimization(algorithm_type, N, kappa_max, omega, eta, c1, c2, lambda_reg=None, mu_hep=None, device=None):
    """
    ...
    elif algorithm_type == 'CNOHEP': # Add print for CNOHEP specific param
        print(f"CNOHEP specific: mu_hep={mu_hep}")

    # Create the swarm
    particles = []
    # Define the model template function here to use the hidden_dim from args
    def create_mlp_model_instance(input_dim, hidden_dim):
        return SimpleMLP(input_dim=input_dim, hidden_dim=hidden_dim)
    ...
    for _ in range(N):
        ...
        elif algorithm_type == 'CNOHEP': # <-- ADD THIS BLOCK
             if mu_hep is None: raise ValueError("mu_hep must be provided for CNOHEP")
             particles.append(ParticleCNOHEP(model_template=create_mlp_model_instance, mu_hep=mu_hep, device=device)) # <-- ADD THIS LINE
    ...
    for i, particle in enumerate(particles):
        ...
        if args.algorithm == 'CNOHEP':
            particle.calculate_fitness(X_train_tensor, y_train_tensor) 
        else:
    ...
    # Use the hidden_dim from args
    best_model = SimpleMLP(input_dim=input_dim, hidden_dim=args.hidden_dim).to(device)
...
if __name__ == "__main__":
    ...
    if args.algorithm == 'CNOGNP':
        lambda_val = args.lambda_reg
        mu_val = None # Not used
    elif args.algorithm == 'CNOHEP': # <-- ADD THIS BLOCK
        lambda_val = None # Not used
        mu_val = args.mu_hep # Use the new mu_hep argument
    else: # CNO or PSO
        lambda_val = None # Not used
        mu_val = None # Not used
    ...
    while True: # <-- Added: Start retry loop
        try:
            excel_dir='log/breast_cancer/{}/{}.xlsx'.format(args.loc,args.loc) #正常对比实验
            ...
            if args.algorithm == 'CNOHEP':
                label = f'CNOHEP_Opt_M{args.mu_hep}_I{args.max_iterations}_P{args.n_particles}_H{args.hidden_dim}_E{args.eta}_W{args.omega}_C1{args.c1}_C2{args.c2}'
