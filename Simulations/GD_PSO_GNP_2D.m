...
% 定义要优化的二维函数 f1
lamda = 0.5
fun_orginal = @(x) sin(x(1).^4) + sin(x(2).^4);
fun = @(x) sin(x(1).^4) + sin(x(2).^4) + lamda * (4*x(1).^3 .* cos(x(1).^4))^2 +lamda * (4*x(2).^3 .* cos(x(2).^4))^2;
gradient_fun = @(x) [4*x(1).^3 .* cos(x(1).^4), 4*x(2).^3 .* cos(x(2).^4)];

% 创建一组坐标点以绘制等高线图和3D图
x1 = linspace(-2, 2, 300); % 在 x1 轴上创建均匀分布的点
x2 = linspace(-2, 2, 300); % 在 x2 轴上创建均匀分布的点
...
% 粒子位置的范围限制
position_min = -1.6;  % 最小位置限制
position_max = 1.6;   % 最大位置限制
...
box on;
grid on;
fileName = sprintf('figures/2-f%d-initial-particles-location-plane-lamda-%.2f.fig', func_num,lamda);
